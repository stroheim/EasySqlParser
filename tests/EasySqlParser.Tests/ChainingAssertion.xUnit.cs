        ...
        public static partial class AssertEx
        {
            ...
            private class DynamicAccessor<T> : DynamicObject
            {
                ...
                public override bool TrySetIndex(SetIndexBinder binder, object[] indexes, object value)
                {
                    try
                    {
                        //var method = typeof(T).GetTypeInfo().GetMethod("Item", TransparentFlags | BindingFlags.SetProperty);
                        //method.Invoke(target, indexes.Concat(new[] {value}).ToArray());
                        var prop = typeof(T).GetTypeInfo().GetProperty("Item", TransparentFlags | BindingFlags.SetProperty);
                        prop.SetValue(null, target, indexes.Concat(new[] { value }).ToArray());
                        //              typeof(T).InvokeMember("Item", 
                        //TransparentFlags | BindingFlags.SetProperty, 
                ...
                public override bool TryGetIndex(GetIndexBinder binder, object[] indexes, out object result)
                {
                    try
                    {
                        //var method = typeof(T).GetTypeInfo().GetMethod("Item", TransparentFlags | BindingFlags.GetProperty);
                        //method.Invoke(target, indexes);
                        var prop = typeof(T).GetTypeInfo().GetProperty("Item", TransparentFlags | BindingFlags.GetProperty);
